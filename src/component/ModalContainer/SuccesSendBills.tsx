...
import { View, StyleSheet, Image, Text, Modal, TouchableOpacity, TouchableWithoutFeedback } from 'react-native';
...
export const SuccesSendBills: FC<IProps> = ({ open, onCancel, friend }) => {
  let uriImage = avatar(friend.uuid);

  return (
    <Modal visible={open} transparent supportedOrientations={['landscape']} onDismiss={onCancel}>
        ...
        <View style={{ backgroundColor: 'rgba(13, 35, 69, 0.6)', flex: 1, display: 'flex' }}>
          ...
          >
            ...
            <View style={{ alignItems: 'center' }}>
              <Text style={styles.textSM}>Передать фишки другу</Text>
              <Image source={{ uri: uriImage }} style={{ width: 60, height: 60, borderRadius: 8, marginTop: 20, marginBottom: 10 }} />
              ...
              <View style={{ backgroundColor: '#75A627', alignItems: 'center', width: '100%', paddingVertical: 9 }}>
                <Text style={styles.textSmall}>Фышки успешно отправлено !</Text>
              </View>
            </View>
          </View>
        </View>
